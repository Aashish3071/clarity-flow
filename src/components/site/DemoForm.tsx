...
export function DemoForm() {
    ...
    return (
      <div className="rounded-2xl border border-border bg-card p-8 shadow-card">
        ...
        <p className="mt-2 text-sm text-ink-soft">
          Your request has been received. Please pick a time below and a specialist will confirm the
          meeting shortly.
        </p>
      </div>
    );
  ...
  return (
    <form
      ...
      noValidate
      className="rounded-2xl border border-border bg-card p-5 shadow-card md:p-7"
    >
      <div className="mb-5">
        <h3 className="text-lg font-semibold text-ink">Tell us about your business</h3>
        <p className="mt-1 text-sm text-ink-soft">
          A short form so the call is useful from minute one.
        </p>
      </div>
      ...
      <div className="grid gap-3.5">
        ...
        <div className="grid gap-3.5 sm:grid-cols-2">
          ...
          <Field label="Preferred contact method" error={errors.contact_pref?.message}>
            <select
              {...register("contact_pref")}
              defaultValue=""
              className={inp(!!errors.contact_pref)}
            >
              <option value="" disabled>
                Select
              </option>
      ...
      <p className="mt-3 text-center text-[11px] text-ink-soft">
        Your details stay private and are only used to prepare the call.
      </p>
    </form>
  );
}
...
function inp(invalid: boolean) {
  return [
    ...
    invalid
      ...
      : "border-border focus:border-[color:var(--brand)]",
  ].join(" ");
}
